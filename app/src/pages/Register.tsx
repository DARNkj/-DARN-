...
import { Eye, EyeOff, Lock, User, Mail, ArrowRight, CheckCircle } from 'lucide-react';
...
const Register = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = context?.register(formData.username, formData.email, formData.password);
        ...
        <div className="glass-card rounded-3xl p-8 md:p-10">
          ...
          <div className="text-center mb-8">
            ...
            <h2 className="text-3xl font-bold text-gray-900 mb-2">
              加入{siteConfig?.siteName?.replace('DARN', '') || '飞影图传'}
            </h2>
            <p className="text-gray-600">创建账号，开始分享你的航空摄影作品</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <label className="flex items-start gap-3 cursor-pointer">
              <div className="relative flex items-center">
                ...
                />
                <div className="w-5 h-5 rounded border-2 border-gray-300 peer-checked:bg-[#0084d9] peer-checked:border-[#0084d9] transition-colors" />
                <CheckCircle className="w-3.5 h-3.5 text-white absolute left-0.5 top-0.5 opacity-0 peer-checked:opacity-100" />
              </div>
              <span className="text-sm text-gray-600">
                我已阅读并同意{' '}
                <Link to="#" className="text-[#0084d9] hover:text-[#0066cc]">
                  用户协议
                </Link>
                {' '}和{' '}
                <Link to="#" className="text-[#0084d9] hover:text-[#0066cc]">
          ...
          <p className="mt-6 text-center text-sm text-gray-600">
            已有账号？{' '}
            <Link to="/login" className="text-[#0084d9] hover:text-[#0066cc] font-medium">
              立即登录
            </Link>
          </p>
        </div>
